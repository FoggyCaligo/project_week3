...
ARTIFACT_ROOT = Path("mk2/artifacts")
...
st.caption("KOLD 데이터셋 + 한국어 ELECTRA 기반 이진 분류 / Streamlit Cloud 서비스용 화면")
...
with tab1:
    ...
    with col2:
        if dataset_summary:
            ...
            c1, c2 = st.columns(2)
            with c1:
                metric_card("전체 샘플 수", f"{dataset_summary.get('total_samples', 0):,}")
            with c2:
                label_info = dataset_summary.get("label_distribution_named", {})
                toxic_count = label_info.get("toxic", 0)
                metric_card("독성 샘플 수", f"{toxic_count:,}")
